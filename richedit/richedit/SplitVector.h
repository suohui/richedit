...
// Modify by Sindy 04/23/2012, add TObjType for auto release obj,and make sure that the body has one element when constructor
...
template <typename T,bool TObjType=true>
class SplitVector {
protected:
	T *body;
	int size;
...
public:
	...
	T &operator[](int position) const {
		assert(position >= 0 && position < lengthBody);
		if (position < part1Length) {
			return body[position];
		} else {
			return body[gapLength + position];
	...
	void EnsureLength(int wantedLength) {
		if (RealLength() < wantedLength) {
			InsertValue(RealLength(), wantedLength - RealLength(), 0);
		}
	}

	/// Insert text into the buffer from an array.
	void InsertFromArray(int positionToInsert, const T* s, int positionFrom, int insertLength) {
		assert((positionToInsert >= 0) && (positionToInsert <= lengthBody));
		if (insertLength > 0) {
	...
	void DeleteRange(int position, int deleteLength) {
		assert((position >= 0) && (position + deleteLength <= lengthBody));
 		...
 		if (TObjType)
 		{
 			for(int i = 0; i < deleteLength; i++)
 			{
				int newPos = position + i;
 				if ( IBaseObj* obj = static_cast<IBaseObj*>((void*)(*this)[newPos]) )
 				{
 					obj->Release();
 					(*this)[newPos] = 0;
 				}
